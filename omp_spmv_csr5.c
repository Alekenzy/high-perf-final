...
// Function to perform SpMV using CSR5 format (parallelized)
void spmv_csr5_parallel(int nrows, CSR5Tile *csr5_tiles, int *row_ptr, int *csr_col_idx, double *csr_values, 
                        double *vector, double *result, int num_tiles, int num_threads) {

    // Parallelize using OpenMP over the tiles
...
int main(int argc, char *argv[]) {
    ...
    int num_threads = atoi(argv[1]);  // Number of threads from command line
    ...
    // Step 5: Perform SpMV using CSR5 format in parallel
    spmv_csr5_parallel(nrows, csr5_tiles, row_ptr, csr_col_idx, csr_values, vector, result, num_tiles, num_threads);
